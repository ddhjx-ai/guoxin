<template>
  <div>
    <Card :bordered="false" dis-hover class="ivu-mt">
      <Form ref="form" :model="searchForm" :label-width="120" label-position="right">
        <Row :gutter="24" type="flex">
          ...
          <Col :span="8">
            <FormItem label="是否生成图片：">
              <Select v-model="searchForm.isPic" placeholder="请选择">
    ...
    >
      <Form ref="createForm" :model="blackFormData" :rules="blackRules" :label-width="120">
        <FormItem label="每天条数：" prop="daily">
          <Input v-model="blackFormData.daily" placeholder="请输入" />
        </FormItem>
        <FormItem label="每周条数：" prop="weekly">
          <Input v-model="blackFormData.weekly" placeholder="请输入" />
        </FormItem>
        <FormItem label="每月条数：" prop="monthly">
          <Input v-model="blackFormData.monthly" type="number" placeholder="请输入" />
        </FormItem>
        <FormItem label="延时时间：" prop="delayTime">
          <Input v-model="blackFormData.delayTime" type="number" placeholder="请输入" />
        </FormItem>
        <FormItem label="禁止登陆IP：" prop="ipNotAllow">
          <Input v-model="blackFormData.ipNotAllow" type="number" placeholder="请输入" />
        </FormItem>
        <FormItem label="允许登陆IP：" prop="isAllow">
          <Input v-model="blackFormData.isAllow" type="number" placeholder="请输入" />
        </FormItem>
        <FormItem label="允许登录IP个数：" prop="ipCount">
          <Input v-model="blackFormData.ipCount" type="number" placeholder="请输入" />
        </FormItem>
        <FormItem label="信息转成图片：" prop="isPic">
          <RadioGroup v-model="blackFormData.isPic">
            <Radio :label="1">是</Radio>
            <Radio :label="0">否</Radio>
          </RadioGroup>
        </FormItem>
        <FormItem label="允许复制：" prop="allowCopy">
          <RadioGroup v-model="blackFormData.allowCopy">
            <Radio :label="1">是</Radio>
            <Radio :label="0">否</Radio>
          </RadioGroup>
        </FormItem>
      ...
      dataList: [
        {
          ...
          monthly: 300,
          delayTime: 1,
          ipNotAllow: "202.118.118.1",
          isAllow: "202.118.118.2",
          ipCount: 11,
          isHunXiao: 1,
          isPic: 1,
          allowCopy: 1
