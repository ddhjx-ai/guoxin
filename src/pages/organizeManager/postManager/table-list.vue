  ...
  <div class="i-table-no-border">
    ...
    <Modal v-model="showCreate" title="编辑规则" :loading="creating" @on-ok="handleCreate">
      <Form ref="create" :model="createData" :rules="createRules" :label-width="120">
        <FormItem label="岗位名称：" prop="desc">
          <Input v-model="createData.desc" placeholder="请输入" />
        </FormItem>
        <FormItem label="岗位编码：" prop="desc">
          <Input v-model="createData.desc" placeholder="请输入" />
        </FormItem>
        <FormItem label="部门：" prop="desc">
          <Select v-model="createData.department" placeholder="请选择" element-id="status1">
...
export default {
  ...
  methods: {
    mockOneData() {
      ...
      item.date = new Date(
        new Date().getTime() - Math.floor(Math.random() * 8640000000)
      );
      item._checked = false;
      return item;
    },
